#this file takes care of pagination in profiles.html page

#paginators import it will allow us to create a multi page website
#PageNotAnInteger, EmptyPage will allow us to set the page number to 1 when the user first go to our profiles page
...
def paginate_profiles(request, profiles, results):
            ...
            #setting up the pages in our profiles.html page
            ...
            paginator = Paginator(profiles, results) #paginator = Paginator(query set , results ) go ahead an paginate the project query set and give us the results here

            try:
                #now paginate the project list page
                #now reset the profiles variable and index it by a specific page only get 6 results (profiles)
                #here we are defining what page do we wanna get from this query set
                #so here we will be getting 6 profiles and we are gonna get the first page those 6 profiles
                profiles = paginator.page(page)
            #the page will not be passed in when the user first enters the profiles_list page
